...
def get_clone(pipe, stateless=False):
    ...
    """Get a new copy of a pipe (transformer or predictor) instance with or without copying internal state.
    
    Parameters
    ----------
    pipe : transformer or predictor
        Pipeline building block
    stateless : bool
        Flag that forces stateless scikit learn transforms when set to True
        
    Returns
    -------
    If stateless is False, a new pipe instance with preserved state if the pipe has 
        a get_clone() method implemented, otherwise a stateless scikit-learn clone
    If stateless is True, a new pipe instance generated by scikit-learn's stateless 
        cloning method: pipe.__class__(**pipe.get_params())
    ...
    """
    if hasattr(pipe, 'get_clone') and stateless == False:

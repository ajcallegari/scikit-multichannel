...
def make_multi_input_classification(n_informative_Xs=3,
    ...
    """
    Wrapper for sklearn's make_classification to make matrix sets with
    informative, weak, and random matrices.

    Parameters
    ---------
    n_informative_Xs: int, the number of informative matrices
    n_weak_Xs: int, the number of weak matrices generated by degrading
        informative matrices with added Gausisan noise
    n_random_Xs: int, the number of random matrices generated by shuffling the
        feature rows of informative matrices
    sklearn_params: parameters for sklearn make_classification, which generates
        the data
    weak_noise_sd: float, standard deviation of the Gaussian noise term used to
    ...
    X_types: list of strings, ordered description of each synthetic matrix as
        'informative', 'weak', or 'random'
    """
    if seed is not None:
        random.seed(seed)
        np.random.seed(seed)
...
def make_multi_input_regression(n_informative_Xs=3,
    ...
    """Get a synthetic regression dataset with multiple feature matrices that
    are either informative, weak, or random.

    Parameters
    ---------
    n_informative_Xs: int, the number of informative input matrices, each with
        information content specified by n_features, n_informative, n_redundant
    n_weak_Xs: int, the number of input matrices generated by degrading
        informative matrices with added Gausisan noise
    n_random_Xs: int, the number of input matrices generated by shuffling the
        feature rows of informative matrices
    weak_noise_sd: float, standard deviation of the Gaussian noise term used
        to generate weak matrices
    seed: int, seed for random number generator

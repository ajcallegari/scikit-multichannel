...
class TestCrossValPredict(unittest.TestCase):
    ...
    def test_predictions(self):
        ...
        self.assertTrue(np.array_equal(sklearn_predictions, pipecaster_predictions),
                        'predictions from pipecaster.model_selection.cross_val_predict did not match sklearn version')

        self.assertTrue(sklearn_auc == pipecaster_auc,
                'auc of pipecaster.model_selection.cross_val_predict did not match sklearn version')
            
        self.assertTrue(pipecaster_auc > 0.5,
                'auc of pipecaster.model_selection.cross_val_predict was not better than random')
...
pipecaster.model_selection.cross_val_predict(clf, X, y, cv = 5, n_jobs = {})'''.format(n_cpus)
            ...
            self.assertTrue(np.array_equal(sklearn_predictions, parallel_predictions), 
                            'the predictions generated by parallel run of pipecaster.model_selection.cross_val_predict \
                             did not match sklearn control')
                        
            self.assertTrue(sklearn_auc == parallel_auc, 
                            'the auc of predictions generated by parallel run of pipecaster.model_selection.cross_val_predict \
                             did not match sklearn control')

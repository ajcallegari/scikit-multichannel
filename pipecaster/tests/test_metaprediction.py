...
class TestMetaClassifier(unittest.TestCase):
    ...
    def test_multi_matrix_voting(self):
        """Test if KNN->MetaClassifier(soft voting) in a pipecaster pipeline gives monotonically increasing accuracy with increasing number of inputs in concordance with Condorcet's jury theorem, and also test hard voting with same pass criterion. Test if accuracy is > 80%.
        """
    ...
    def test_multi_matrices_svm_metaclassifier(self):
        """Test if KNN classifier->MetaClassifier(SVC) in a pipecaster pipeline gives monotonically increasing accuracy with increasing number of inputs, and test if accuracy is > 80%.
        """        
        n_jobs = multiprocessing.cpu_count()
        if n_jobs > 1:
            # shut off warnings because ray and redis generate massive numbers
...
class TestMetaRegressor(unittest.TestCase):
    
    def test_singe_matrix_mean_voting(self, seed=42):
        """Determine if KNN->MetaRegressor(mean voting) in a pipecaster pipeline gives identical predictions to sklearn KNN on training data
        """
        ...
        self.assertTrue(np.array_equal(rgr_predictions, mrgr_predictions), 
                        'mean voting MetaRegressor failed to reproduce sklearn result on single matrix prediction task')
        
    def test_singe_matrix_median_voting(self, seed=42):
        """Determine if KNN->MetaRegressor(median voting) in a pipecaster pipeline gives identical predictions to sklearn KNN on training data
        """
        X, y = make_regression(n_samples=100, n_features=20, n_informative=10, random_state=seed)
        ...
        layer1[:] = rgr
        layer2 = mrgr.get_next_layer()
        layer2[:] = MetaRegressor('median voting')
        mrgr.fit([X], y)
        mrgr_predictions = mrgr.predict([X])
        self.assertTrue(np.array_equal(rgr_predictions, mrgr_predictions), 
                        'median voting MetaRegressor failed to reproduce sklearn result on single matrix prediction task')
        
    def test_multi_matrix_voting(self, seed = 42, verbose=1):
        """Determine if KNN->MetaRegressor(voting) in a pipecaster pipeline gives monotonically increasing accuracy with increasing number of inputs
        """
    ...
    def test_multi_matrix_SVR_stacking(self, seed = 42, verbose=0):
        """Determine if KNN->MetaRegressor(voting) in a pipecaster pipeline gives monotonically 
           increasing accuracy with increasing number of inputs
        """

...
from skmultichannel.transform_wrappers import make_transformer
...
from skmultichannel.ensemble_learning import Ensemble, SoftVotingClassifier, \
...
class TestEnsembleMetaprediction(unittest.TestCase):
    ...
    def test_compare_to_StackingClassifier(self, verbose=0, seed=42):
        ...
        sm_score_all = np.mean(cross_val_score(mclf, [X], y, cv=5, n_processes=5))
        ...
        sm_score_informative = np.mean(cross_val_score(mclf, [X], y, cv=5, n_processes=5))
        ...
        if verbose > 0:
            ...
            print('Ensemble informative predictors: {}'.format(sm_score_informative))
            print('Ensemble all predictors: {}'.format(sm_score_all))
        ...
        self.assertTrue(np.round(sm_score_all, 2) == np.round(sm_score_informative, 2),
                        'Ensemble accuracy is not same for all classifiers and informative classifiers.')
        tolerance_pct = 5
        self.assertTrue(sm_score_all >= sk_score_informative * (1 - tolerance_pct / 100.0),

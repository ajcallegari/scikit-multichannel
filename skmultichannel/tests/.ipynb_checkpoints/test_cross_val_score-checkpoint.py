...
class TestCrossValScore(unittest.TestCase):
    ...
    def test_single_input_classification(self):
        sm_scores = sm_cross_validation.cross_val_score(
            self.clf, self.X_cls, self.y_cls, score_method='predict_proba',
            scorer=roc_auc_score,
            cv=self.cv, n_processes=1)
        self.assertTrue(np.array_equal(self.cls_scores, sm_scores),
    ...
    def test_multi_input_classification(self):
        ...
        self.assertTrue(np.array_equal(self.cls_scores, sm_scores), 'classifier scores from skmultichannel.cross_validation.cross_val_score did not match sklearn control (multi input predictor)')
    ...
    def test_single_input_regression(self):
        sm_scores = sm_cross_validation.cross_val_score(self.rgr, self.X_rgr, self.y_rgr, scorer=explained_variance_score,
                                                        cv=self.cv, n_processes=1)
        self.assertTrue(np.array_equal(self.rgr_scores, sm_scores), 'regressor scores from skmultichannel.cross_validation.cross_val_score did not match sklearn control (single input predictor)')

    def test_multi_input_regression(self):
        mrgr = MultichannelPipeline(n_channels=1)
        mrgr.add_layer(self.rgr)
        sm_scores = sm_cross_validation.cross_val_score(mrgr, [self.X_rgr], self.y_rgr, scorer=explained_variance_score,
                                                       cv=self.cv, n_processes=1)
        self.assertTrue(np.array_equal(self.rgr_scores, sm_scores), 'regressor scores from skmultichannel.cross_validation.cross_val_score did not match sklearn control (multi input predictor)')
...
import skmultichannel.cross_validation'''
            TEST_CODE = '''
skmultichannel.cross_validation.cross_val_score(mclf, [X], y, cv = 5, n_processes = 1)'''
            t_serial = timeit.timeit(setup = SETUP_CODE,
                                  stmt = TEST_CODE,
                                  globals = locals(),
                                  number = 5)
            TEST_CODE = '''
skmultichannel.cross_validation.cross_val_score(mclf, [X], y, cv = 5, n_processes = {})'''.format(n_cpus)

...
from skmultichannel.testing_utils import make_multi_input_classification
from skmultichannel.testing_utils import make_multi_input_regression

from skmultichannel.multichannel_pipeline import MultichannelPipeline
from skmultichannel.ensemble_learning import SoftVotingMetaClassifier
from skmultichannel.ensemble_learning import HardVotingMetaClassifier
from skmultichannel.ensemble_learning import AggregatingMetaRegressor
from skmultichannel.ensemble_learning import MultichannelPredictor
from skmultichannel.ensemble_learning import ChannelEnsemble
from skmultichannel.cross_validation import cross_val_score
from skmultichannel.score_selection import RankScoreSelector
import skmultichannel.transform_wrappers as transform_wrappers
...
class TestMultichannelClassification(unittest.TestCase):
    ...
    def test_single_matrix_soft_voting(self):
        """
        Determine if KNN->ChannelClassifier(soft voting) in a skmultichannel
    ...
    def test_single_matrix_hard_voting(self):
        """
        Determine if KNN->ChannelClassifier(hard voting) in a skmultichannel
        pipeline gives identical predictions to sklearn KNN on training data.
        """
        X, y = make_classification(n_samples=100, n_features=20,
                                   n_informative=10, class_sep=5,
    ...
    def test_multi_matrix_voting(self, verbose=0):
        """
        Test if KNN->ChannelClassifier(soft voting) in a skmultichannel pipeline
    ...
    def test_multi_matrices_svm_metaclassifier(self, seed=42, verbose=0):
        """
        Test if KNN classifier->ChannelClassifier(SVC) in a skmultichannel
...
class TestMultiChannelRegression(unittest.TestCase):
    ...
    def test_single_matrix_mean_voting(self, seed=42):
        """
        Determine if KNN->ChannelRegressor(mean voting) in a skmultichannel
        pipeline gives identical predictions to sklearn KNN on training data
        """
    ...
    def test_multi_matrix_SVR_stacking(self, verbose=0, seed=42):
        """
        Determine if KNN->ChannelRegressor(SVR()) in a skmultichannel pipeline
        gives monotonically increasing accuracy with increasing number of

...
class TestInputSelectors(unittest.TestCase):
    ...
    @staticmethod
    def _select_synthetic(input_selector, n_Xs=20, n_informative_Xs=5, n_weak_Xs=5, weak_noise_sd=10):
    ...
    @staticmethod
    def _test_weak_strong_input_discrimination(input_selector, n_weak = 5, n_strong = 5, weak_noise_sd = 25):
    ...
    @staticmethod
    def _test_weak_input_detection(input_selector, n_weak = 5, n_strong = 5, weak_noise_sd = 25):
        n_Xs = 2*(n_weak + n_strong)
        n_informative_hits, n_random_hits, n_weak_hits = TestInputSelectors._select_synthetic(input_selector, 
                                                                             n_Xs =  n_Xs,
                                                                             n_informative_Xs = n_strong,
    ...
    def test_SelectKBestInputs_weak_strong_input_discrimination(self):
        ...
        passed = TestInputSelectors._test_weak_strong_input_discrimination(input_selector, n_weak = 5, 
                                                                           n_strong = 5, weak_noise_sd = 25)
        self.assertTrue(passed)
        
    def test_SelectKBestInputs_weak_input_detection(self):
        k = 10
        input_selector = pc.SelectKBestInputs(score_func=f_classif, aggregator=np.sum, k=k)
        passed = TestInputSelectors._test_weak_input_detection(input_selector, n_weak = 5, 
                                                               n_strong = 5, weak_noise_sd = 20)
        self.assertTrue(passed)     
